namespace CryptoExchange.API
{
    ...
    /// <summary>
    /// An abstraction over <see cref="HttpClient"/>.  Relases lingering connections more quickly for faster performance.
    ...
    public sealed class RestClient : IRestClient, IDisposable
    {
        ...
        /// <summary>
        /// Sends a <c>POST</c> request with the given <paramref name="requestMessage"/>
        /// </summary>     
        /// <exception cref="ArgumentNullException"/>
        public Task<HttpResponseMessage> PostAsync(HttpRequestMessage requestMessage)
            => PostAsync(requestMessage, CancellationToken.None);

        /// <summary>
        /// Sends a <c>POST</c> request with the given <paramref name="requestMessage"/> with the given <paramref name="cToken"/>.
        /// </summary>     
        /// <exception cref="ArgumentNullException"/>
        public Task<HttpResponseMessage> PostAsync(HttpRequestMessage requestMessage, CancellationToken cToken = default(CancellationToken))
            => SendAsync(requestMessage, cToken);
        ...
        {
            Requires.NotNullOrEmptyOrWhiteSpace(uri);
        ...
        {
            Requires.NotNullOrEmptyOrWhiteSpace(uri, nameof(uri));
        ...
        public static IRestClient CreateCryptoApiRestClient()
        {
            ...
            var defaultHeaders = new Dictionary<string, string>
            {
                ...
                //,{HttpRequestHeader.UserAgent.ToString(), "TradeSatoshi API .NET Client" }
            };
            return new RestClient(defaultRequestHeaders: defaultHeaders, handler: handler);
        }
    }
}
